<template>
  <Dialog v-model="visible" :title="data.no || t('reimbursement.detail')" width="1000px">
    ...
    <div class="mb-12px mt-20px flex items-center justify-between">
      <h3 class="m-0">{{ t('reimbursement.actionRecords') }}</h3>
      <el-button v-if="data.processInstanceId" link type="primary" @click="viewProcess">
...
const viewProcess = () =>
  push({ name: 'BpmProcessInstanceDetail', query: { id: data.value.processInstanceId } })
