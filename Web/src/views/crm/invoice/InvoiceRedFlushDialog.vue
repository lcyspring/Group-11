<template>
  <Dialog v-model="visible" :title="t('invoice.redFlush')" width="600px">
    <el-descriptions :column="2" border class="mb-16px">
      ...
      <el-descriptions-item :label="t('invoice.remainingRedAmount')">{{
        remaining
      }}</el-descriptions-item>
    </el-descriptions>
    <el-form ref="formRef" :model="formData" :rules="rules" label-width="120px">
      <el-form-item :label="t('invoice.redInvoiceNo')" prop="invoiceNo"
        ><el-input v-model="formData.invoiceNo" maxlength="50"
      /></el-form-item>
      <el-form-item :label="t('invoice.redAmountForm')" prop="amount"
        ><el-input-number
          ...
          class="!w-full"
      /></el-form-item>
      <el-form-item :label="t('invoice.invoiceDate')" prop="invoiceDate"
        ><el-date-picker
      ...
      ></el-form-item>
      <el-form-item :label="t('invoice.redReason')" prop="reason"
        ><el-input v-model="formData.reason" type="textarea" maxlength="500" show-word-limit
      /></el-form-item>
    </el-form>
    <template #footer
      ><el-button type="danger" :loading="loading" @click="submit">{{
        t('invoice.confirmRedFlush')
      }}</el-button
...
type InvoiceRedFlushForm = Omit<InvoiceApi.InvoiceRedFlushReqVO, 'invoiceDate'> & {
...
const rules = {
  ...
  invoiceDate: [{ required: true, message: t('invoice.invoiceDateRequired'), trigger: 'change' }],
  handlerUserId: [{ required: true, message: t('invoice.handlerRequired'), trigger: 'change' }],
  reason: [{ required: true, message: t('invoice.redReasonRequired'), trigger: 'blur' }]
}
const open = async (row: InvoiceApi.InvoiceVO) => {
  visible.value = true
  invoice.value = row
...
const submit = async () => {
  ...
  loading.value = true
  try {
    await InvoiceApi.redFlushInvoice({
      ...formData.value,
      invoiceDate: toInvoiceEpochMillis(formData.value.invoiceDate)
    })
    message.success(t('invoice.redFlushSuccess'))
    visible.value = false
    emit('success')
  } finally {
    loading.value = false
