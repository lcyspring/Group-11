<template>
  <Dialog v-model="visible" :title="title" width="840px">
    ...
    <el-form ref="formRef" v-loading="loading" :model="formData" :rules="rules" label-width="120px">
      ...
      <el-card v-if="summary" shadow="never" class="mb-16px">
        <el-row :gutter="12" class="text-center">
          <el-col
            ...
            :key="item.label"
            :span="Math.floor(24 / summaryItems.length)"
          >
      ...
      <el-form-item :label="t('invoice.title')" prop="title"
        ><el-input v-model="formData.title" maxlength="200"
      /></el-form-item>
      <el-row :gutter="16">
        <el-col :span="12"
          ><el-form-item :label="t('invoice.taxNo')" prop="taxNo"
            ><el-input v-model="formData.taxNo" /></el-form-item
        ></el-col>
        <el-col :span="12"
          ><el-form-item :label="t('invoice.email')" prop="email"
            ><el-input v-model="formData.email" /></el-form-item
        ></el-col>
      </el-row>
      <template v-if="formData.type === INVOICE_TYPE.VAT_SPECIAL">
        <el-row :gutter="16">
          <el-col :span="12"
            ><el-form-item :label="t('invoice.registeredAddress')" prop="registeredAddress"
              ><el-input v-model="formData.registeredAddress" /></el-form-item
          ></el-col>
          <el-col :span="12"
            ><el-form-item :label="t('invoice.registeredPhone')" prop="registeredPhone"
              ><el-input v-model="formData.registeredPhone" /></el-form-item
          ></el-col>
        </el-row>
        <el-row :gutter="16">
          <el-col :span="12"
            ><el-form-item :label="t('invoice.bankName')" prop="bankName"
              ><el-input v-model="formData.bankName" /></el-form-item
          ></el-col>
          <el-col :span="12"
            ><el-form-item :label="t('invoice.bankAccount')" prop="bankAccount"
              ><el-input v-model="formData.bankAccount" /></el-form-item
          ></el-col>
        </el-row>
      </template>
      <el-form-item :label="t('invoice.content')" prop="content"
        ><el-input
...
const title = computed(() =>
  mode.value === 'create' ? t('invoice.createDraft') : t('invoice.updateDraft')
)
...
const specialRequired = (_rule: unknown, value: string, callback: (error?: Error) => void) => {
  if (formData.value.type === INVOICE_TYPE.VAT_SPECIAL && !value?.trim())
...
const rules = reactive({
  ...
  amount: [required('invoice.amountRequired')],
  title: [required('invoice.titleRequired')],
  content: [required('invoice.contentRequired')],
  taxNo: [{ validator: specialRequired, trigger: 'blur' }],
  registeredAddress: [{ validator: specialRequired, trigger: 'blur' }],
  registeredPhone: [{ validator: specialRequired, trigger: 'blur' }],
  bankName: [{ validator: specialRequired, trigger: 'blur' }],
...
const draftFields = () => ({
  ...
  registeredAddress: formData.value.registeredAddress,
  registeredPhone: formData.value.registeredPhone,
...
const loadSummary = async () => {
  summary.value = formData.value.contractId
    ? await InvoiceApi.getContractSummary(formData.value.contractId)
    : undefined
}
const open = async (type: 'create' | 'update', id?: number) => {
  ...
  try {
    const [contractPage, userList] = await Promise.all([
      ContractApi.getContractPage({ pageNo: 1, pageSize: 100, auditStatus: 20 }),
    ...
    if (id) formData.value = await InvoiceApi.getInvoice(id)
      ...
      formData.value = {
        ...
        handlerUserId: useUserStore().getUser.id,
        type: INVOICE_TYPE.VAT_ORDINARY,
        amount: 0.01,
        title: '',
        content: ''
      } as unknown as InvoiceApi.InvoiceVO
    await loadSummary()
  } finally {
    loading.value = false
  }
...
const submit = async () => {
  ...
  try {
    if (mode.value === 'create')
      await InvoiceApi.createInvoice({ contractId: formData.value.contractId, ...draftFields() })
