<template>
  <ContentWrap>
    <el-form ref="queryFormRef" :model="queryParams" inline class="-mb-15px">
      <el-form-item :label="t('invoice.applicationNo')"
        ><el-input v-model="queryParams.no" clearable @keyup.enter="query"
      /></el-form-item>
      <el-form-item :label="t('invoice.invoiceNo')"
        ><el-input v-model="queryParams.invoiceNo" clearable @keyup.enter="query"
      /></el-form-item>
      ...
      ></el-form-item>
      <el-form-item :label="t('invoice.direction')"
        ><el-select v-model="queryParams.direction" clearable @change="query"
          ><el-option :label="t('invoice.directionBlue')" :value="1" /><el-option
            :label="t('invoice.directionRed')"
            :value="-1" /></el-select
      ></el-form-item>
      <el-form-item :label="t('invoice.type')"
        ><el-select v-model="queryParams.type" clearable @change="query"
          ><el-option :label="t('invoice.typeOrdinary')" :value="1" /><el-option
            :label="t('invoice.typeSpecial')"
            :value="2" /></el-select
      ></el-form-item>
      <el-form-item :label="t('invoice.customer')"
        ><el-select v-model="queryParams.customerId" clearable filterable @change="query"
          ><el-option
      ...
      ></el-form-item>
      <el-form-item :label="t('invoice.invoiceDate')"
        ><el-date-picker
      ...
      <el-form-item>
        ...
        <el-button
          ...
          >{{ t('invoice.createDraft') }}</el-button
        >
        <el-button
  ...
  <ContentWrap>
    ...
    <el-table v-loading="loading" :data="list" stripe>
      ...
      />
      <el-table-column :label="t('invoice.contract')" min-width="190" show-overflow-tooltip
        ><template #default="{ row }"
          >{{ row.contractNo }} · {{ row.contractName }}</template
        ></el-table-column
      >
      ...
      <el-table-column :label="t('invoice.status')" min-width="110"
        ...
        ></el-table-column
      >
      <el-table-column :label="t('invoice.amount')" prop="amount" align="right" min-width="120"
        ><template #default="{ row }">{{ money(row.amount) }}</template></el-table-column
      >
      <el-table-column
        :label="t('invoice.redAmount')"
        prop="redAmount"
        align="right"
        min-width="120"
        ><template #default="{ row }">{{ money(row.redAmount) }}</template></el-table-column
      >
      ...
      <el-table-column :label="t('common.action')" fixed="right" width="140">
        <template #default="{ row }">
          <TableActions v-if="hasInvoiceActions(row)" mode="menu">
            ...
            >
            <el-button
              v-if="canIssueInvoice(row.status, row.direction)"
              v-hasPermi="['crm:invoice:issue']"
              link
              type="success"
              @click="issueRef.open(row)"
              >{{ t('invoice.issue') }}</el-button
            >
            <el-button
              v-if="canRedFlushInvoice(row.status, row.direction)"
              v-hasPermi="['crm:invoice:red-flush']"
              link
              type="warning"
              @click="redRef.open(row)"
              >{{ t('invoice.redFlush') }}</el-button
            >
            <el-button
              v-if="canVoidInvoice(row.status)"
              v-hasPermi="['crm:invoice:void']"
              link
              type="danger"
              @click="voidRef.open(row)"
              >{{ t('invoice.void') }}</el-button
            >
            <el-button
              v-if="canEditInvoice(row.status, row.direction)"
...
const statusTag = (value: number) =>
  (({ 0: 'info', 10: 'success', 20: 'warning', 30: 'danger', 40: 'info' }) as Record<number, any>)[
    value
  ]
...
const getList = async () => {
  loading.value = true
  try {
    const data = await InvoiceApi.getInvoicePage({ ...queryParams, sceneType: sceneType.value })
...
const remove = async (row: InvoiceApi.InvoiceVO) => {
...
onMounted(async () => {
  customers.value = await CustomerApi.getCustomerSimpleList()
  await getList()
})
onActivated(getList)
</script>
