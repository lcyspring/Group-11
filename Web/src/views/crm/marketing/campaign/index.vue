  ...
  <ContentWrap>
    <el-form :inline="true" class="mb-10px">
      <el-form-item :label="t('crm.marketing.name')"
        ><el-input v-model="query.name" clearable @keyup.enter="reload"
      /></el-form-item>
      <el-form-item
        ><el-button type="primary" @click="reload">{{ t('common.query') }}</el-button
        ><el-button @click="openCreate">{{ t('common.create') }}</el-button></el-form-item
      >
    </el-form>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column fixed="right" :label="t('common.action')" width="140">
        <template #default="scope">
          <TableActions mode="menu">
            <el-button
              ...
              >{{ t('crm.marketing.start') }}</el-button
            >
            <el-button
    ...
    <el-dialog v-model="dialogVisible" :title="t('crm.marketing.createCampaign')" width="560px">
      <el-form :model="form" label-width="110px"
        ><el-form-item :label="t('crm.marketing.code')"
          ><el-input v-model="form.code" /></el-form-item
        ><el-form-item :label="t('crm.marketing.name')"
          ><el-input v-model="form.name" /></el-form-item
        ><el-form-item :label="t('crm.marketing.ownerUserId')"
          ><el-input-number v-model="form.ownerUserId" :min="1" /></el-form-item
        ><el-form-item :label="t('crm.marketing.startTime')"
          ><el-date-picker v-model="form.startTime" type="datetime" /></el-form-item
        ><el-form-item :label="t('crm.marketing.endTime')"
          ><el-date-picker v-model="form.endTime" type="datetime" /></el-form-item
      ></el-form>
      <template #footer
...
const form = reactive<any>({ code: '', name: '', ownerUserId: 1, startTime: '', endTime: '' })
const reload = async () => {
  loading.value = true
  try {
    const res = await MarketingApi.getCampaignPage(query)
    list.value = res.list || []
...
const openCreate = () => {
  Object.assign(form, { code: '', name: '', ownerUserId: 1, startTime: '', endTime: '' })
  dialogVisible.value = true
}
const save = async () => {
  await MarketingApi.saveCampaign({ ...form, relations: [] })
...
const complete = async (id: number) => {
  await ElMessageBox.confirm(t('crm.marketing.confirmComplete'))
  await MarketingApi.completeCampaign({ id, summary: t('crm.marketing.completedByUser') })
  reload()
}
reload()
</script>
