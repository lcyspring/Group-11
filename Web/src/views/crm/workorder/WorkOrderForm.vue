<template>
  <Dialog v-model="visible" :title="title">
    <el-form ref="formRef" :model="formData" :rules="rules" label-width="100px">
      ...
      <el-row :gutter="16">
        <el-col :span="12"><el-form-item :label="t('workOrder.type')" prop="type"><el-select v-model="formData.type" class="w-full"><el-option v-for="item in types" :key="item.value" :label="item.label" :value="item.value" /></el-select></el-form-item></el-col>
      ...
      <el-row :gutter="16">
        ...
        <el-col :span="12"><el-form-item :label="t('workOrder.handler')" prop="handlerUserId"><el-select v-model="formData.handlerUserId" class="w-full" filterable :disabled="mode === 'update'"><el-option v-for="item in users" :key="item.id" :label="item.nickname" :value="item.id" /></el-select></el-form-item></el-col>
      </el-row>
      <el-row :gutter="16">
...
const rules = reactive({
  ...
  description: [{ required: true, message: t('workOrder.descriptionRequired'), trigger: 'blur' }]
})
...
const open = async (type: 'create' | 'update', id?: number) => {
  ...
  } else formData.value = { type: 1, priority: 2, sourceType: 0, title: '', description: '' } as WorkOrderApi.WorkOrderVO
}
const submit = async () => {
