<template>
  <ContentWrap :title="t('oa.event.title')">
    <el-form inline :model="form"><el-form-item :label="t('oa.event.titleLabel')"><el-input v-model="form.title" /></el-form-item><el-form-item><el-select v-model="viewMode" @change="changeView"><el-option value="day" :label="t('oa.event.day')" /><el-option value="week" :label="t('oa.event.week')" /><el-option value="month" :label="t('oa.event.month')" /></el-select><el-date-picker v-model="anchor" type="date" value-format="YYYY-MM-DD" @change="changeView" /><el-button type="primary" @click="save">{{ form.id ? t('common.update') : t('common.create') }}</el-button><el-button @click="reset">{{ t('common.reset') }}</el-button></el-form-item></el-form>
