<template>
  <ContentWrap :title="t('oa.event.title')">
    <el-form inline :model="form"><el-form-item :label="t('oa.event.titleLabel')"><el-input v-model="form.title" /></el-form-item><el-form-item><div class="flex flex-wrap gap-8px"><el-date-picker v-model="eventRange" type="datetimerange" value-format="YYYY-MM-DD HH:mm:ss" /><el-button type="primary" @click="save">{{ form.id ? t('oa.event.update') : t('oa.event.create') }}</el-button><el-button @click="reset">{{ t('common.reset') }}</el-button></div></el-form-item></el-form>
