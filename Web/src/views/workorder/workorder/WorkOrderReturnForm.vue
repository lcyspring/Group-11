<template>
  <Dialog v-model="dialogVisible" title="退回工单" width="500px">
    <el-form
      ref="formRef"
    ...
    >
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="退回原因" prop="result">
            <el-input
...
defineOptions({ name: 'WorkOrderReturnForm' })
...
const formRules = reactive({
  result: [{ required: true, message: '请输入退回原因', trigger: 'blur' }]
})
const formRef = ref()
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    await WorkOrderApi.returnWorkOrder({
