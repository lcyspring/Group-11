...
5. 项目依赖在编译容器运行时下载到 Podman 命名卷；Host 不安装项目 JDK、Maven、Node、pnpm 或
   HBuilderX，也不生成项目 `node_modules`。
...
Spring Boot 的 `application.yaml`、pnpm 的 `pnpm-lock.yaml`/`pnpm-workspace.yaml` 属于框架或
包管理器原生文件，不受本 ADR 的 Podman KDL 唯一入口约束。
