# CRM-FEATURE-063：营销 Provider KDL 显式 provision

## 目标

把 CRM 真实短信/邮件发送所需的系统渠道、账号和模板纳入 `deploy.sh` 使用的 KDL 契约，同时保留
...
- `managed`：由 KDL 幂等创建或更新稳定键对应的渠道、账号和模板。
...
- 示例 KDL 保留 `not-configured`，真实秘密只进入被 Git 忽略的本机配置；
- `managed` 只管理明确稳定键，不批量清理其他供应商账号；
- 本功能验证数据库配置闭环，不伪造供应商审核、真实发送或回执结果。

## 当前结论

已完成 KDL、部署入口和数据库幂等 provision。`create-only` 重复执行、`managed` 更新、`disabled`
