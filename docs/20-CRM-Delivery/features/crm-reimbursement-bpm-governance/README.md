...
- 角色代码、审批账号和模型字段均来自 YAML，不在 Java 或 BPMN 中硬编码用户编号；
- 审批角色所需的 BPM 待办查询、审批、流程详情和报销读取权限由 YAML 列出，脚本按权限码解析菜单及全部祖先菜单后合并授权；
- 本机凭据 YAML 独立忽略，可上传配置仅保留 `CHANGE_ME`；
...
复制 `podman/config/bpm-provision.example.yaml` 为本机忽略的配置，填入账号后执行：
