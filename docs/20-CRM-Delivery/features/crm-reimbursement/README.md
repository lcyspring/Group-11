...
审批模型由 `podman/provision-bpm-model.sh` 使用显式 YAML 治理，审批人按 `crm_finance_approver` 角色动态解析。当前运行样本：`BX202607-0001` 审批通过，`BX202607-0002` 驳回修订重提后通过，`BX202607-0003` 由发起人取消；当前无遗留报销运行实例。
