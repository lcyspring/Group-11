...
旧 `deploy.sh` 的 `full` 和 `rebuild-*` 同时执行 `podman build` 与容器替换。编译、镜像封装、部署三个
责任被压进同一个状态操作，导致“替换容器”可能意外消费 Host 上尚未验收的产物，单组件模式名称也
无法判断究竟是构建镜像还是替换服务。

## 修复

- 新增 YAML-only `build-images.sh`，只封装已有产物；
- `deploy.sh` 删除全部源码产物检查、Containerfile 读取和 `podman build`；
...
`deploy.sh` 中不得出现 `podman build` 或读取 JAR、`dist-prod`、H5 产物的逻辑；镜像封装脚本不得调用
编译入口或 `deploy.sh`。两个脚本均保持单 YAML 参数契约，并分别提供 `check` 配置。
