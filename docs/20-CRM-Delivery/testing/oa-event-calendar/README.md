...
编辑时间与视图范围隔离，以及清空日期后查询全部日程。专项测试 6/6。

## Ubuntu 26.04 构建门禁

2026-07-19 使用上述命令完成 Ubuntu 26.04 专项测试、ESLint、生产构建和三语言包校验。

已生成 `coverage/verify-oa-event.lcov`。当前 6 项属于 Vue/接口源码契约测试，Deno 报告中实际计入的是测试
进程加载的工具配置，因此报告显示的 100% 不代表日程 SFC 的浏览器运行时覆盖率。可验证结果为专项
测试 6/6、BPM 后端 65/65、ESLint 0 warning、Vite 8 生产构建成功和三语言包完整性通过。
