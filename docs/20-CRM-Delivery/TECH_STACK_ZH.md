...
| 管理端 | Vue 3.5.12、Vite 8.1.5、TypeScript 5.7.3、Element Plus 2.11.1 | `Web/package.json`、`Web/deno.lock` |
| 前端工具链 | Deno 2.9.3、Deno Test/Coverage | 公共工具链镜像、`podman/Containerfile.build-ubuntu`、构建 KDL |
...
- Spring Boot 继续使用原生 `application.yaml`；Web 与 Mall 使用原生 `deno.json` 和 `deno.lock`。
...
| Host JDK、Maven、Node、pnpm、Deno、HBuilderX 构建 | 两个公开 Ubuntu 26.04 工具链镜像 |
...
| 不支持软链接目录的暂存复制 | Deno 在容器卷中生成标准 `node_modules` 链接结构 |
