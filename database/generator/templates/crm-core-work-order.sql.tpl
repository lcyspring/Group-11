...
BEGIN
  ...
  WHILE i <= __CUSTOMERS__ DO
    ...
    INSERT INTO crm_customer
      (name,owner_user_id,owner_time,pool_status,pool_entry_time,pool_previous_owner_user_id,
       pool_reason,pool_cycle_count,lock_status,deal_status,lifecycle_status,
    ...
    VALUES
      (CONCAT(@demo_batch,'-CUS-',LPAD(i,6,'0')),
       IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),NULL,record_owner),event_time,
       IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),1,0),
       IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),event_time,NULL),
       IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),record_owner,NULL),
       IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),'MANUAL_PUT_POOL',NULL),
       IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),1,0),
       b'0',IF(MOD(i,4)=2,b'1',b'0'),
    ...
    INSERT INTO crm_customer_owner_record
      ...
      VALUES(customer_id,record_owner,IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),record_owner,NULL),
             IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),NULL,record_owner),
             IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),1,3),
             IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),'MANUAL_PUT_POOL','INITIAL_ASSIGN'),@demo_batch,
             CAST(@demo_owner AS CHAR),CAST(@demo_owner AS CHAR),b'0',@demo_tenant);
    IF i<=(__CUSTOMERS__-__PUBLIC_CUSTOMERS__) THEN
    ...
    INSERT INTO crm_contact
      (name,customer_id,owner_user_id,mobile,email,sex,birthday,master,primary_contact,remark,
       creator,create_time,updater,update_time,deleted,tenant_id)
      VALUES(CONCAT(@demo_batch,'-CONTACT-',LPAD(i,6,'0')),customer_id,
       IF(i>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),NULL,CAST(record_owner AS CHAR)),
  ...
  -- Add non-primary contacts independently from customer count, so contact pages and
  -- customer 360 views exercise one-to-many relationships.
  SET i=__CUSTOMERS__+1;
  WHILE i<=__CONTACTS__ DO
    SET customer_seq=MOD(i-1,__CUSTOMERS__)+1;
    SELECT id INTO customer_id FROM demo_customer_ids WHERE seq=customer_seq;
    SELECT id INTO record_owner FROM demo_owner_ids WHERE seq=MOD(customer_seq,5);
    SET event_time=DATE_ADD(@demo_start,INTERVAL MOD(__SEED__+i*39,@demo_span_days) DAY);
    INSERT INTO crm_contact
      (name,customer_id,owner_user_id,mobile,email,sex,birthday,master,primary_contact,remark,
       creator,create_time,updater,update_time,deleted,tenant_id)
      VALUES(CONCAT(@demo_batch,'-CONTACT-',LPAD(i,6,'0')),customer_id,
       IF(customer_seq>(__CUSTOMERS__-__PUBLIC_CUSTOMERS__),NULL,CAST(record_owner AS CHAR)),
       CONCAT('17',LPAD(MOD(__SEED__+i,1000000000),9,'0')),
       CONCAT('contact.',__SEED__,'.',i,'@example.invalid'),MOD(i,2),
       DATE_ADD('1980-01-01',INTERVAL MOD(i*97,12000) DAY),b'0',b'0',
       CONCAT('generated-batch:',@demo_batch),CAST(@demo_owner AS CHAR),event_time,
       CAST(@demo_owner AS CHAR),event_time,b'0',@demo_tenant);
    SET contact_id=LAST_INSERT_ID();
    IF customer_seq<=(__CUSTOMERS__-__PUBLIC_CUSTOMERS__) THEN
      INSERT INTO crm_permission
        (biz_type,biz_id,user_id,level,creator,create_time,updater,update_time,deleted,tenant_id)
        VALUES(3,contact_id,record_owner,1,CAST(@demo_owner AS CHAR),NOW(),
               CAST(@demo_owner AS CHAR),NOW(),b'0',@demo_tenant);
  ...
  WHILE i <= __CLUES__ DO
    ...
    INSERT INTO crm_clue
      ...
      VALUES(CONCAT(@demo_batch,'-CLUE-',LPAD(i,6,'0')),b'0',
       IF(i<=__PUBLIC_CLUES__,NULL,record_owner),
       IF(i<=__PUBLIC_CLUES__,NULL,event_time),
       IF(i<=__PUBLIC_CLUES__,1,0),
       IF(i<=__PUBLIC_CLUES__,event_time,NULL),
       NULL,IF(i<=__PUBLIC_CLUES__,'CREATE_UNASSIGNED',NULL),
       IF(i<=__PUBLIC_CLUES__,'固定种子公共线索样本',NULL),IF(i<=__PUBLIC_CLUES__,1,0),b'0',NULL,
    ...
    INSERT INTO crm_clue_owner_record
      (clue_id,previous_owner_user_id,new_owner_user_id,type,source,reason,
       creator,create_time,updater,update_time,deleted,tenant_id)
      VALUES(business_id,NULL,IF(i<=__PUBLIC_CLUES__,NULL,record_owner),
       IF(i<=__PUBLIC_CLUES__,1,3),IF(i<=__PUBLIC_CLUES__,'CREATE_UNASSIGNED','INITIAL_ASSIGN'),
       CONCAT('generated-batch:',@demo_batch),CAST(@demo_owner AS CHAR),NOW(),
       CAST(@demo_owner AS CHAR),NOW(),b'0',@demo_tenant);
    IF i>__PUBLIC_CLUES__ THEN
      INSERT INTO crm_permission
        (biz_type,biz_id,user_id,level,creator,create_time,updater,update_time,deleted,tenant_id)
  ...
  -- The base administrator participates in every non-owned core object. This is
  -- distinct from ownership and deliberately exercises the "我参与的" scene.
  INSERT INTO crm_permission
    (biz_type,biz_id,user_id,level,creator,create_time,updater,update_time,deleted,tenant_id)
  SELECT 2,c.id,@demo_owner,3,CAST(@demo_owner AS CHAR),NOW(),CAST(@demo_owner AS CHAR),NOW(),b'0',@demo_tenant
    FROM crm_customer c WHERE c.tenant_id=@demo_tenant AND c.deleted=b'0'
     AND c.name LIKE CONCAT(@demo_batch,'-CUS-%') AND c.pool_status=0 AND c.owner_user_id<>@demo_owner;
  INSERT INTO crm_permission
    (biz_type,biz_id,user_id,level,creator,create_time,updater,update_time,deleted,tenant_id)
  SELECT 3,c.id,@demo_owner,3,CAST(@demo_owner AS CHAR),NOW(),CAST(@demo_owner AS CHAR),NOW(),b'0',@demo_tenant
    FROM crm_contact c WHERE c.tenant_id=@demo_tenant AND c.deleted=b'0'
     AND c.name LIKE CONCAT(@demo_batch,'-CONTACT-%') AND c.owner_user_id IS NOT NULL
     AND c.owner_user_id<>@demo_owner;
  INSERT INTO crm_permission
    (biz_type,biz_id,user_id,level,creator,create_time,updater,update_time,deleted,tenant_id)
  SELECT 1,c.id,@demo_owner,3,CAST(@demo_owner AS CHAR),NOW(),CAST(@demo_owner AS CHAR),NOW(),b'0',@demo_tenant
    FROM crm_clue c WHERE c.tenant_id=@demo_tenant AND c.deleted=b'0'
     AND c.name LIKE CONCAT(@demo_batch,'-CLUE-%') AND c.pool_status=0 AND c.owner_user_id<>@demo_owner;
  INSERT INTO crm_permission
    (biz_type,biz_id,user_id,level,creator,create_time,updater,update_time,deleted,tenant_id)
  SELECT 4,b.id,@demo_owner,3,CAST(@demo_owner AS CHAR),NOW(),CAST(@demo_owner AS CHAR),NOW(),b'0',@demo_tenant
    FROM crm_business b WHERE b.tenant_id=@demo_tenant AND b.deleted=b'0'
     AND b.name LIKE CONCAT(@demo_batch,'-BUS-%') AND b.owner_user_id<>@demo_owner;
