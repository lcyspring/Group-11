...
| `podman/` | KDL 驱动的编译、镜像封装、部署与运维入口 |
| `docs/20-CRM-Delivery/` | 当前开发的功能、测试、覆盖率、Bug 和交付文档 |

文档入口：

- [当前交付导航](docs/20-CRM-Delivery/README.md)
- [技术栈基线与清理边界](docs/20-CRM-Delivery/TECH_STACK_ZH.md)
- [文档治理规范](docs/20-CRM-Delivery/DOCUMENTATION_GOVERNANCE_ZH.md)
- [人工验收指南](docs/20-CRM-Delivery/user-guide/README.md)

## 环境要求

宿主机只需 Git、rootless Podman 及其网络组件。公开工具链镜像为：

- `ghcr.io/elel-code/group-11-build-ubuntu:26.04-deno-2.9.3`：Server、InitService、Web、测试和覆盖率；
...
所有入口只接受一个 KDL 配置文件路径。配置字段、示例和运行模式详见
[`podman/README_ZH.md`](podman/README_ZH.md) 与
[`podman/config/KDL_FIELDS_ZH.md`](podman/config/KDL_FIELDS_ZH.md)。
...
```bash
bash podman/stop.sh podman/config/cleanup-stop.example.kdl
```
...
如需供局域网其他主机访问，应在运行 KDL 中显式设置 `network.host_address: 0.0.0.0`，并按
实际访问源配置 CORS；不要依赖脚本硬编码或宿主环境变量。

## 数据库与演示数据

`deploy.sh` 根据运行 KDL 选择 bootstrap、compatibility 和 dataset manifest。新空库执行建表，
已有库只执行幂等兼容迁移；演示数据通过 `mysql.dataset_mode` 的 `preserve`、`insert` 或 `replace`
集中表达。`replace` 要求 manifest 第一项先清理旧数据集，再插入新数据；生产环境应使用
`preserve` 且不选择演示数据集。
