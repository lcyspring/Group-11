...
[[ $# -eq 1 ]] || {
    printf 'Usage: bash ./tests/acceptance/verify-crm-customer-import.sh <config.yaml>\n' >&2
    exit 2
}
yaml_config_init "$1"
...
GENERATOR_IMAGE="$(yaml_require image.generator)"
PREFIX="$(yaml_require test.customer_prefix)"
